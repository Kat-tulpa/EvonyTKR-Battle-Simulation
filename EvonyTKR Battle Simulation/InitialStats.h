...
class InitialStats {
private:
	Stats stats[TIER_COUNT][TYPE_COUNT];
...
public:
	...
	// Getters
	const Attribute get(const unsigned int tier, 
		const Type type, const ATTRIBUTE attribute) {
		return stats[tier][type].get(attribute);
	}

	const Distance getRange(const unsigned int tier) {
		switch (tier) {
		case T15:
		case T14:
		case T13:
	...
	const Distance getRange(const Type type) {
		if (type == TYPE_RANGED)
			return 500;
		return 50;
	}
	...
	// Setters
	void set(const unsigned int tier, const Type type, const Attribute atk, 
		const Attribute def, const Attribute hp) {
		stats[tier][type].set(atk, def, hp);
	}
